...
Task(
  ...
  🎯 검토 포인트:
  1. **요구사항 커버리지**: 모든 요구사항이 PLAN에 반영되었는가?
  2. 마일스톤 크기가 적절한가?
  3. 순서가 논리적인가?
  4. 빠진 단계가 있는가?"
)
...
**중요**: sub-ai 호출 시 반드시 원본 요구사항 목록을 함께 전달하여 커버리지 검증 가능하게 할 것
```

**피드백 처리**:

- 🎯 요구사항 누락: 즉시 해당 마일스톤에 추가 → Task로 sub-ai 재호출
...
**요구사항 제외 시**: 기술적 불가능, 범위 초과 등 명시적 사유 필요

## 승인 기준

**승인 조건**:

- 요구사항 커버리지: 100% (모든 요구사항 반영)
- Critical 이슈: 0개
- Important 이슈: 0-1개
- 명시적 승인 메시지
...
**참고**: 사용자가 요청한 것은 특별한 결격 사유 없이는 모두 PLAN에 포함해야 함
